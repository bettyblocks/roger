defmodule Roger.Partition.Consumer do
  ...
  def terminate(reason, state) do
    Enum.each(state.queues, fn q ->
      IO.inspect(q, label: "closing queue")

      if q.consumer_tag do
        AMQP.Basic.cancel(q.channel, q.consumer_tag)
      end
  ...
  def handle_info({:basic_cancel_ok, %{consumer_tag: consumer_tag}}, state) do
    if Map.has_key?(state.closing, consumer_tag) do
      IO.inspect(state.closing, label: "closing")
  ...
  defp do_reconfigure(state, new_queues) do
    ...
    closing =
      ...
      |> Enum.map(fn q ->
        ...
        IO.inspect(q, label: "state.queue")
